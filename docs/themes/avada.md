...
sidebar_position: 1
title: Avada
description: How to set up SyteSlyders with the Avada theme and Fusion Builder.
---

# Avada Configuration

Avada with Fusion Builder is fully supported by SyteSlyders. This guide walks you through placing hero sliders, setting up text overlays, and handling common layout scenarios.
...
SyteSlyders injects dynamic text into your page builder elements using CSS class names. Add these classes to your Fusion Builder elements:

### Text Areas

1. Add a **Text Block** element below the slider shortcode.
2. In the element settings, add the CSS class `slyders-hero-text-area-1` to use it as the primary headline.
3. For a secondary tagline, add another Text Block with the class `slyders-hero-text-area-2`.

### CTA Button

1. Add a **Button** element in the same column.
2. In the element settings, add the CSS class `slyders-hero-cta`.
3. SyteSlyders will automatically update the button text and link on each slide transition.
...
If your hero text overlays appear at the bottom of the column instead of centered, add the following CSS in **SyteSlyders > Custom CSS > Global CSS**:
...
Fusion Builder offers built-in visibility controls for columns (show/hide on desktop, tablet, mobile). You can combine these with SyteSlyders' view-locked shortcodes for full responsive control:

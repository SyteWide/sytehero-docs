...
Manage AI-generated media. Requires the fal.ai integration to be enabled (see [AI Image Generation](/docs/features/ai-image-generation)).

- **AI Media** — Browse all images generated by SyteHero's AI tools. Each card shows the thumbnail, model used, cost tier, prompt preview, and whether the image is currently in use across the site. Select images and bulk-delete unused ones. A confirmation dialog is required before any deletion. Use **Backfill** to retroactively tag images generated before this tracking feature was added.
- **Generate** — Placeholder for future text-to-image generation tools.
...
## Settings: Email Cron Status & Reset

The Settings page includes a diagnostics section showing the current state of email cron scheduling:
